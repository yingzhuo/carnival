 ...
 * @since 1.6.25
 */
public class TypeStringTokenSupport implements HandlerMethodArgumentResolver {

    @Override
    public boolean supportsParameter(MethodParameter parameter) {
        return parameter.getParameterType() == StringToken.class;
    }

    @Override
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) {
        try {
            return (StringToken) RestfulSecurityContext.current().getToken();
        } catch (Exception e) {
            return null;
        }
    }
