...
import static external.Util.firstNonNull;
...
abstract class Connection extends BasePubSub implements Closeable {
    ...
    protected final BlockingQueue<String> respQueue = new ArrayBlockingQueue<>(1);
    ...
    private static final Set<String> nonFatalErrors = Collections.unmodifiableSet(new HashSet<>(
    ...
    public synchronized void connect(Config config) throws IOException {
        ...
        log.debug("serverConfig:{}", response);
        ...
        log.info("connected {} msgTimeout:{} heartbeatInterval:{} maxRdyCount:{}", host, msgTimeout, heartbeatInterval, maxRdyCount);
    ...
    private void wrapCompression(ServerConfig serverConfig, StreamPair streams) {
    ...
    private void sendAuthorization(ServerConfig serverConfig) throws IOException {
        if (serverConfig.getAuthRequired() != null && serverConfig.getAuthRequired()) {
            ...
            log.info("authorization response:{}", authResponse);
    ...
    private void checkHeartbeat() {
        try {
            ...
            if (isDead) {
                log.info("heartbeat failed, closing connection:{}", toString());
                close();
            }
        } catch (Exception e) {
            log.error("problem checking heartbeat, will probably time out soon. {}", toString(), e);
        }
    }
    ...
    private String readResponse() throws IOException {
        ...
        if (frameType == 0) {       //response
            ...
            if (nonFatalErrors.contains(error)) {
                log.warn("non fatal nsqd error:{} probably due to message timeout", error);
            } else {
    ...
    private void read() {
        try {
            ...
            if (isReading) {
                ...
                close();
                log.error("read thread exception. con:{}", toString(), e);
            }
        }
        log.debug("read loop done {}", toString());
    }
    ...
    private synchronized void receivedHeartbeat() {
        try {
            ...
            log.error("receivedHeartbeat error", t);
        }
    }
    ...
    public synchronized void flushAndClose() {
        try {
            ...
            log.error("flushAndClose error", e);
        }
        close();
    }
