 ...
 * @since 1.10.14
 */
abstract class AbstractGraphQLFactoryBean implements BeanPostProcessor {

    protected final Map<String, DataFetcher<?>> dataFetcherMap = new HashMap<>();
    ...
    private Entry createEntry(final Object bean, final Query query, final Method method) {
        ...
        // 无参数
        if (parameterCount == 0) {
            dataFetcher = (DataFetcher<?>) env -> method.invoke(bean);
        ...
        for (int i = 0; i < method.getParameterCount(); i++) {
            ...
            // TODO: check argument annotation
        ...
        dataFetcher = (DataFetcher<?>) env -> {
            ...
            for (Argument a : arguments) {
                Object p = env.getArgument(a.value());
                parameters.add(p);
            }
